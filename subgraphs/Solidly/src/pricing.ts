...
export function findFtmPerToken(token: Token, stable: boolean): BigDecimal {
  ...
  for (let i = 0; i < WHITELIST.length; ++i) {
    ...
    if (pairAddress.toHexString() != ADDRESS_ZERO) {
      ...
      // return token1 per our token * FTM per token 1
      if (pair.token0 == token.id && pair.reserveFTM.gt(MINIMUM_LIQUIDITY_THRESHOLD_FTM)) {
        if (token1) {
          return pair.token1Price.times(token1.derivedFTM as BigDecimal)
        }
      }

      // return token0 per our token * FTM per token 0
      if (pair.token1 == token.id && pair.reserveFTM.gt(MINIMUM_LIQUIDITY_THRESHOLD_FTM)) {
        if (token0) {
          return pair.token0Price.times(token0.derivedFTM as BigDecimal)
        }
      }
    }
  }
...
export function getTrackedLiquidityUSD(
  ...
  let price0 = token0DerivedFTM.times(bundle.ftmPrice)
  let price1 = token1DerivedFTM.times(bundle.ftmPrice)
